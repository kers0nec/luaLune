...
export function validateSignup({ username, email, password }) {
  if (!emailRegex.test(String(email || "").trim())) return "Enter a valid email address.";
    ...
    async signup({ username, email, password }) {
      ...
      if (remote) {
        const { data, error: authError } = await remote.auth.signUp({
          email: address,
          password,
          options: { data: { username: clean }, emailRedirectTo: process.env.LUALUNE_AUTH_REDIRECT || undefined },
        });
