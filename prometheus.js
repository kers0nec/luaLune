...
async function factory() {
  if (!factoryPromise) factoryPromise=(async()=>{
    if(!fs.existsSync(ROOT)) throw new Error('LuaLune Lune Obfuscator engine is missing.');
...
async function obfuscate(source,{preset='medium',antiTamper=true}={}) {
  ...
  if(Buffer.byteLength(source)>500_000) throw new Error('Source exceeds the 500 KB obfuscation limit.');
  if(antiTamper) source=wrapWithAntiTamper(source);
  const lua=await (await factory()).createEngine();
  try {
    lua.global.set('LUALUNE_SOURCE',source);
    ...
    return `-- Protected by LuaLune 2.0 | Lune Obfuscator\\n${result}`;
  } catch(error) {
